...
RBUTrainingScheduleListForm form = (RBUTrainingScheduleListForm)request.getAttribute("trainingScheduleListForm");
    String emplid = form.getEmplid();
    String oldclassid = form.getOldCourseID();
    System.out.println("emplid = " + emplid);
     System.out.println("oldclassid = " + form.getOldCourseID());
...
<html>
    ...
    <body onload="window.focus(); document.CancelTraining.reason.value='';document.all.reason.focus();">    
        <form name="CancelTraining"   method="post"  target="" action="getEmployeeDetails">       
        <input type="hidden" name="commandchangetime" value="cancelTraining">
        <input type="hidden" name="emplid" value="<%=emplid%>">
        <input type="hidden" name="oldclassid" value="<%=oldclassid%>">  	
