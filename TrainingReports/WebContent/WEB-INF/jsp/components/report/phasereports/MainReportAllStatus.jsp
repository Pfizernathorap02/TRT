...
<table class="blue_table_without_border">
	<tr>
		<td colspan="2"><img src="<%=AppConst.IMAGE_DIR%>/spacer.gif">
		</td>
	</tr>
	<tr>
		<td colspan="2">
			<div class="breadcrumb">
				...
				/
				<%} /*2020 Q3:if condition added by muzees for null pointer exception in activitylink*/%><a href="listReportAllStatus?activitypk=<%=wc.getActivityId()%>"><%=wc.getPageName()%></a>
			</div>
		</td>
	</tr>
	<tr>
		<%if (wc.getErrorMsg().length() > 0) {%>
		<td colspan="2"><%=wc.getErrorMsg()%></td>
		<%} else {%>
		<td><inc:include-wc component="<%=wc.getArea1()%>" /></td>
		<td colspan="3" width="60%">
			<% if(wc.getArea3() != null) { %> <inc:include-wc
				component="<%=wc.getArea3()%>" /> <% } %>
		</td>
		<%}%>
	</tr>
	<tr>
			...
			<div class="chartscontrol_area">
				<table class="blue_table_without_border" align="left">
					<tr>
						...
						<td>
							<!-- Added for TRT Phase 2 - HQ Users --> <% User user = wc.getUser();
                    if(!user.isHQUser()){%>
