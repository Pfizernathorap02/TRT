...
<%@ page import="com.pfizer.webapp.wc.components.report.ReportList"%>
...
function sortables_init() {
    ...
    <body>
        <p>
           <%
         ReportList wc = (ReportList)request.getAttribute(ListReportWpc.ATTRIBUTE_NAME);         
  ...
  <tr>
        ...
        String trmColor="";
         ...
         <td><%=b%><%=Util.toEmptyNBSP(emp.getTeamCode())%><%=bb%></td>
				...
				<td><%=b%><a href="<%=AppConst.APP_ROOT%>/overview/detailreportPDFHS?emplid=<%=emp.getEmplId()%>"><%=emp.getLastName()%> </a><%=bb%></td>
				<td><%=b%><a href="<%=AppConst.APP_ROOT%>/overview/detailreportPDFHS?emplid=<%=emp.getEmplId()%>"><%=emp.getFirstName()%></a><%=bb%></td>
