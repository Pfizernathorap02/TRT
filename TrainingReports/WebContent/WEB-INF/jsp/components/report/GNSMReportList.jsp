...
function sortables_init() {
    ...
    <body> 
        <p>
           <%
        GNSMReportListWc wc = (GNSMReportListWc)request.getAttribute(ListReportWpc.ATTRIBUTE_NAME);         
...
function submitEmail() {
	...
	var ccString = '';
	var subjectStr = '&subject=Geodon National Sales Meeting Follow-up';	
...
function unCheckAll() {
	...
	<tr>
        ...
        <div>
	<div id="table_inst">
		<img src="<%=AppConst.APP_ROOT%>/resources/images/excel.gif" width="15px">&nbsp;<a href="<%=AppConst.APP_ROOT%>/GNSM/listReportGNSM?downloadExcel=true&type=<%=qString.getType()%>&section=<%=qString.getSection()%>">Download to Excel</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a  href="<%=AppConst.APP_ROOT%>/GNSM/listReportGNSM?type=<%=qString.getType()%>&section=<%=qString.getSection()%>">Back to default sort</a>
	</div>
  ...
  <tr>
        ...
        String trmColor="";
         ...
         <td><%=(emp.getTeamCode()==null)?"":emp.getTeamCode()%></td>
				...
				<td><a href="<%=AppConst.APP_ROOT%>/PWRA/employeeDetail/getEmployeeDetail?emplid=<%=emp.getEmplId()%>&m1=GNSM"><%=emp.getLastName()%> </a></td>
				<td><a href="<%=AppConst.APP_ROOT%>/PWRA/employeeDetail/getEmployeeDetail?emplid=<%=emp.getEmplId()%>&m1=GNSM"><%=emp.getFirstName()%></a></td>
				<td><%=emp.getRole()%></td>
				<td><%=emp.getEmplId()%></td>
                <%if("Overall".equalsIgnoreCase(type)){
