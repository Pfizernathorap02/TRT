...
function sortables_init() {
    ...
    <body>
        <p>
           <%
         PDFHSReportList wc = (PDFHSReportList)request.getAttribute(ListReportWpc.ATTRIBUTE_NAME);         
...
function submitEmail() {
	...
	var ccString = '';
	var subjectStr = '&subject=Powers Driving Force Home Study Follow-up';	
...
function unCheckAll() {
	...
	<tr>
        ...
        <div>
	<div id="table_inst">
		<img src="<%=AppConst.APP_ROOT%>/resources/images/excel.gif" width="15px">&nbsp;<a href="<%=AppConst.APP_ROOT%>/PWRA/listreport?downloadExcel=true&type=<%=qString.getType()%>&section=<%=qString.getSection()%>">Download to Excel</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a  href="<%=AppConst.APP_ROOT%>/PWRA/listreport?type=<%=qString.getType()%>&section=<%=qString.getSection()%>">Back to default sort</a>
	</div>
...
<table cellspacing="0" id="employee_table" width="100%" class="employee_table">
  <tr>
        ...
        String trmColor="";
         ...
         <td><%=b%><%=Util.toEmptyNBSP(emp.getTeamCode())%><%=bb%></td>
				<td><%=b%><%=(emp.getDistrictDesc()==null)?"":emp.getDistrictDesc()%><%=bb%></td>
                	
				<td><%=b%><a href="<%=AppConst.APP_ROOT%>/PWRA/employeeDetail/getEmployeeDetail?emplid=<%=emp.getEmplId()%>&m1=PDF&m2=HS"><%=emp.getLastName()%> </a><%=bb%></td>
				<td><%=b%><a href="<%=AppConst.APP_ROOT%>/PWRA/employeeDetail/getEmployeeDetail?emplid=<%=emp.getEmplId()%>&m1=PDF&m2=HS"><%=emp.getFirstName()%></a><%=bb%></td>
