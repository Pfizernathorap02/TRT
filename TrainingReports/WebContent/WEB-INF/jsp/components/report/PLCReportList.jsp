...
function sortables_init() {
    ...
    <body>
        <p>
           <%
         PLCReportList wc = (PLCReportList)request.getAttribute(ListReportWpc.ATTRIBUTE_NAME);         
...
function submitEmail() {
	...
	var ccString = '';
	var subjectStr = '&subject=POWERS Driving Force PLC Report Follow-up';	
...
function unCheckAll() {
	...
	<tr>
        ...
        <div>
	<div id="table_inst">
		<img src="<%=AppConst.APP_ROOT%>/resources/images/excel.gif" width="15px">&nbsp;<a href="<%=AppConst.APP_ROOT%>/PWRA/listReportPLC?downloadExcel=true&type=<%=qString.getType()%>&section=<%=qString.getSection()%>">Download to Excel</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a  href="<%=AppConst.APP_ROOT%>/PWRA/listReportPLC?type=<%=qString.getType()%>&section=<%=qString.getSection()%>">Back to default sort</a>
	</div>
  ...
  <tr>
        ...
        String trmColor="";
         ...
         <td><%=b%><%=(emp.getTeamCode()==null)?"":emp.getTeamCode()%><%=bb%></td>
				...
				<td><%=b%><a href="<%=AppConst.APP_ROOT%>/PWRA/employeeDetail/getEmployeeDetail.do?emplid=<%=emp.getEmplId()%>&m1=PDF&m2=PLC"><%=emp.getLastName()%> </a><%=bb%></td>
				<td><%=b%><a href="<%=AppConst.APP_ROOT%>/PWRA/employeeDetail/getEmployeeDetail.do?emplid=<%=emp.getEmplId()%>&m1=PDF&m2=PLC"><%=emp.getFirstName()%></a><%=bb%></td>
			 --%>
				<td><%=b%><a href="<%=AppConst.APP_ROOT%>/PWRA/employeeDetail/getEmployeeDetail?emplid=<%=emp.getEmplId()%>&m1=PDF&m2=PLC"><%=emp.getLastName()%> </a><%=bb%></td>
				<td><%=b%><a href="<%=AppConst.APP_ROOT%>/PWRA/employeeDetail/getEmployeeDetail?emplid=<%=emp.getEmplId()%>&m1=PDF&m2=PLC"><%=emp.getFirstName()%></a><%=bb%></td>
