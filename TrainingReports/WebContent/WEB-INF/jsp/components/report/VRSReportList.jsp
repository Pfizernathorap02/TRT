...
<%@ page import="com.pfizer.webapp.wc.components.report.VRSReportListWc"%>
...
function sortables_init() {
    ...
    <body> 
        <p>
           <%
        VRSReportListWc wc = (VRSReportListWc)request.getAttribute(ListReportWpc.ATTRIBUTE_NAME);         
...
function submitEmail() {
	...
	var ccString = '';
	var subjectStr = '&subject=Vista Rx Spiriva Follow-up';	
...
function unCheckAll() {
	...
	<tr>
        ...
        <div>
	<div id="table_inst">
		<img src="<%=AppConst.APP_ROOT%>/resources/images/excel.gif" width="15px">&nbsp;<a href="<%=AppConst.APP_ROOT%>/VRS/listReportVRS?downloadExcel=true&type=<%=qString.getType()%>&section=<%=qString.getSection()%>">Download to Excel</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a  href="<%=AppConst.APP_ROOT%>/VRS/listReportVRS?type=<%=qString.getType()%>&section=<%=qString.getSection()%>">Back to default sort</a>
	</div>
  ...
  <tr>
        ...
        String trmColor="";
         ...
         <td><%=(emp.getTeamCode()==null)?"":emp.getTeamCode()%></td>
				...
				<td><a href="<%=AppConst.APP_ROOT%>/PWRA/employeeDetail/getEmployeeDetail?emplid=<%=emp.getEmplId()%>&m1=VRS"><%=emp.getLastName()%> </a></td>
				<td><a href="<%=AppConst.APP_ROOT%>/PWRA/employeeDetail/getEmployeeDetail?emplid=<%=emp.getEmplId()%>&m1=VRS"><%=emp.getFirstName()%></a></td>
				<td><%=emp.getRole()%></td>
				<td><%=emp.getEmplId()%></td>
                <%if("Overall".equalsIgnoreCase(type)){
                    ...
                    %>
                    <td bgcolor="#FFF39C"><%=Util.toEmptyNA((String)empHashMap.get("VISTASPRV1"))%></td>
                    <td bgcolor="#FFF39C"><%=Util.toEmptyNA((String)empHashMap.get("VISTASPRV2"))%></td>
                    <%--<td bgcolor="#FFF39C"><%=b%><%=Util.toEmptyNA((String)empHashMap.get("RVTO"))%><%=bb%></td>--%>
                    <td bgcolor="#FFF39C"><%=Util.toEmptyNA((String)empHashMap.get("VISSPRVATT"))%></td>
                    <td ><%=qString.getSection()%></td>
                
                <%}else{
                    %>
                    <td bgcolor="#FFE87C"><%=emp.getCourseStatus()%></td>
                    <%
                }%>
