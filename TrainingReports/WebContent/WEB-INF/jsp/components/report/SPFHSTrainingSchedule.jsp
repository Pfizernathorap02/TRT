...
<html>
    <head>
        <title>
            SPF - Training Schedule Summary Report
        </title>
    </head>
    ...
    <body>
        ...
        <div style="margin-left:10px;margin-right:10px">
            <div id="table_inst">
               <%--  <img src="<%=AppConst.APP_ROOT%>/resources/images/excel.gif" width="15px">&nbsp;<a href="<%=AppConst.APP_ROOT%>/SPF/SPFHSReportTrainingSchedule.do?downloadExcel=true">Download to Excel</a>
                --%>
                 <img src="<%=AppConst.APP_ROOT%>/resources/images/excel.gif" width="15px">&nbsp;<a href="<%=AppConst.APP_ROOT%>/SPF/SPFHSReportTrainingSchedule?downloadExcel=true">Download to Excel</a>
        ...
        <%if (downloadExcel) {%>
        <tr>
            <td colspan="5"><b>SPF - Training Schedule Summary Report</b></td>
        </tr>
        <tr></tr>
        <%}%>
        <tr>
        ...
        <tr <%=trClass%> bgcolor="<%=trmColor%>">
            ...
            <td>
                <%if (!downloadExcel) {%>
                <%-- <a href="<%=AppConst.APP_ROOT%>/SPF/SPFHSReportTrainingScheduleEmplList.do?<%=ClassFilterForm.FIELD_PRODUCT%>=<%=oEmpReport.getProductCode()%>&<%=ClassFilterForm.FIELD_STARTDATE%>=<%=strStartDate%>&<%=ClassFilterForm.FIELD_ENDDATE%>=<%=strEndDate%>&<%=ClassFilterForm.FIELD_TEAMCD%>=<%=oEmpReport.getTeamCode()%>">
             --%>
             <a href="<%=AppConst.APP_ROOT%>/SPF/SPFHSReportTrainingScheduleEmplList?<%=ClassFilterForm.FIELD_PRODUCT%>=<%=oEmpReport.getProductCode()%>&<%=ClassFilterForm.FIELD_STARTDATE%>=<%=strStartDate%>&<%=ClassFilterForm.FIELD_ENDDATE%>=<%=strEndDate%>&<%=ClassFilterForm.FIELD_TEAMCD%>=<%=oEmpReport.getTeamCode()%>">
        ...
        <tr bgcolor="#ffd699">
            ...
            <td><b>
            <%if (!downloadExcel) {%>
          <%--   <a href="<%=AppConst.APP_ROOT%>/SPF/SPFHSReportTrainingScheduleEmplList.do?<%=ClassFilterForm.FIELD_PRODUCT%>=<%=oEmpReport.getProductCode()%>">
           --%>
             <a href="<%=AppConst.APP_ROOT%>/SPF/SPFHSReportTrainingScheduleEmplList?<%=ClassFilterForm.FIELD_PRODUCT%>=<%=oEmpReport.getProductCode()%>">
        ...
        <tr bgcolor="#d6ebad">
            <td><b>Total SPF Trainees</b></td>
            <td colspan="3">&nbsp;</td>            
            <td><b>
            <%if (!downloadExcel) {%>
            <%-- <a href="<%=AppConst.APP_ROOT%>/SPF/SPFHSReportTrainingScheduleEmplList.do"> --%>
           <a href="<%=AppConst.APP_ROOT%>/SPF/SPFHSReportTrainingScheduleEmplList">
            <%}%>
            <%=grandTotal%>            
            <%if (!downloadExcel) {%></a><%}%>
            </b></td>
