...
<%
	ReportFilterSelectWc wc = (ReportFilterSelectWc) request
			.getAttribute(ReportFilterSelectWc.ATTRIBUTE_NAME);
...
<style type="text/css">
@import
	url(/TrainingReports/resources/js/jscalendar-1.0/calendar-blue.css);
</style>
<script type="text/javascript"
	src="/TrainingReports/resources/js/jscalendar-1.0/calendar.js"></script>

<script type="text/javascript"
	src="/TrainingReports/resources/js/jscalendar-1.0/lang/calendar-en.js"></script>

<script type="text/javascript"
	src="/TrainingReports/resources/js/jscalendar-1.0/calendar-setup.js"></script>
...
<table class="basic_table" border="0" width="0" style="">
	<form name="selOptionalFieldsForm" action="<%=wc.getPostUrl()%>"
		method="post" class="form_basic">
		<%--/* Added for Phase 1 by Meenakshi */ --%>
		<tr>
			<td valign="top" width="30%">
				<table style="">
					<tr>
						<td colspan="3"><h5 align="center">Search Criteria</h5></td>

					</tr>
					<tr>
						<td width="50px">Status:</td>
						<td width="100px" style=""><select
							id="<%=wc.getTerritoryFilterForm().FIELD_CHKSTATUS%>"
							name="<%=wc.getTerritoryFilterForm().FIELD_CHKSTATUS%>"
							style="width: 120px;">
								<%=HtmlBuilder.getOptionsFromLabelValue(tff.getStatusList(),
					tff.getChkStatus())%>
								<%
					...
					<!-- padding added by shindo edge -->
					<tr>
						<td nowrap style="padding-bottom: 20;">From Date:</td>
						<td>
							<table>
								<tr>
									<td><input class="text" type="text"
										name="<%=wc.getTerritoryFilterForm().FIELD_FROMDATE%>"
										id="<%=wc.getTerritoryFilterForm().FIELD_FROMDATE%>"
										value="<%=wc.getTerritoryFilterForm().getFromDate()%>"
										size="12" readonly></td>
						<!-- valign removed shindo Edge release -->
									<td valign=""><input type=image
										src="/TrainingReports/resources/images/calendar.jpg"
										id="fromDate_Id" border="0" height="17" width="20"
										alt="Select From Date" value="Change Date" name="changeDate">
                                ...
                                Calendar.setup({
									...
									<td><img
										src="/TrainingReports/resources/images/cancel.gif"
										alt="Clear From Date" onmouseover="this.style.cursor='hand'"
										height="17" width="20"
										onclick="clearDates('<%=wc.getTerritoryFilterForm().FIELD_FROMDATE%>');">
									</td>
								</tr>
								<tr>
									<td><div class="text" id="msg"
					...
					<tr>
						<td nowrap style="padding-bottom: 3;">To Date:</td>
						<td>
							<table>
								<tr>
								<td><input class="text" type="text"
										name="<%=wc.getTerritoryFilterForm().FIELD_TODATE%>"
										id="<%=wc.getTerritoryFilterForm().FIELD_TODATE%>"
										value="<%=wc.getTerritoryFilterForm().getToDate()%>"
										size="12" readonly></td>
										<!-- valign removed shindo edge-->
									<td valign=""><input type=image
										src="/TrainingReports/resources/images/calendar.jpg"
										id="toDate_Id" border="0" height="17" width="20"
										alt="Select From Date" value="Change Date" name="changeDate">
                            ...
                            </script></td> <!-- shindo changed id to toDate_Id to calendar setup -->
									<td><img
										src="/TrainingReports/resources/images/cancel.gif"
										alt="Clear To Date" onmouseover="this.style.cursor='hand'"
										height="17" width="20"
										onclick="clearDates('<%=wc.getTerritoryFilterForm().FIELD_TODATE%>');">
									</td>
								</tr>
					...
					<tr>
						<!-- left margin added shindo edge-->
						<td align="center" colspan="3">
							<!--edited for HQ user--> <input name="" type="image"
							src="/TrainingReports/resources/images/btn_getreport.gif"
							style="margin-top: 20px;margin-left: 20px;"
							onclick="javascript:concatSelectedFields();concatSelectedHQFields();return validate('<%=wc.getTerritoryFilterForm().FIELD_FROMDATE%>','<%=wc.getTerritoryFilterForm().FIELD_TODATE%>');concatSelectedFields();">
 ...
 %> <input type="hidden" name="newSet" value="" /> <!-- shindo height change edge -->
				<div style="width: 190px; height: 150px; overflow-y: scroll;">
					<table class="blue_table" width="100%" style="">
						<tr class="even">
							<td><input type="checkbox" name="chkBox" id="chkBox"
								value="EMPLID"
								<%if (selList != null) {
				for (int i = 0; i < selList.size(); i++) {
					if ("EMPLID".equals(selList.get(i).toString())) {%>
								checked <%}
				}
			}%>></td>
							<td>Employee ID</td>
						...
						<tr class="odd">
							<td><input type="checkbox" name="chkBox" id="chkBox"
								...
								<%if (selList != null) {
				for (int i = 0; i < selList.size(); i++) {
					if ("Sex".equals(selList.get(i).toString())) {%>
								checked <%}
				}
			}%>></td>
							<td>Gender</td>
						...
						<tr class="even">
							<td><input type="checkbox" name="chkBox" id="chkBox"
								...
								<%if (selList != null) {
				for (int i = 0; i < selList.size(); i++) {
					if ("GEO_DESC".equals(selList.get(i).toString())) {%>
								checked <%}
				}
			}%>></td>
						...
						<tr class="odd">
							<td><input type="checkbox" name="chkBox" id="chkBox"
								...
								<%if (selList != null) {
				for (int i = 0; i < selList.size(); i++) {
					if ("GUID".equals(selList.get(i).toString())) {%>
								checked <%}
				}
			}%>></td>
							<td>GUID</td>
						...
						<tr class="even">
							<td><input type="checkbox" name="chkBox" id="chkBox"
								...
								<%if (selList != null) {
				for (int i = 0; i < selList.size(); i++) {
					if ("HIRE_DATE".equals(selList.get(i).toString())) {%>
								checked <%}
				}
			}%>></td>
							<td>Hire Date</td>
						...
						<tr class="odd">
							<td><input type="checkbox" name="chkBox" id="chkBox"
								...
								<%if (selList != null) {
				for (int i = 0; i < selList.size(); i++) {
					if ("MEMAILADDRESS".equals(selList.get(i).toString())) {%>
								checked <%}
				}
			}%>></td>
						...
						<tr class="even">
							<td><input type="checkbox" name="chkBox" id="chkBox"
								...
								<%if (selList != null) {
				for (int i = 0; i < selList.size(); i++) {
					if ("PROMOTION_DATE".equals(selList.get(i).toString())) {%>
								checked <%}
				}
			}%>></td>
						...
						<tr class="odd">
							<td><input type="checkbox" name="chkBox" id="chkBox"
								...
								<%if (selList != null) {
				for (int i = 0; i < selList.size(); i++) {
					if ("STATE".equals(selList.get(i).toString())) {%>
								checked <%}
				}
			}%>></td>
							<td>Regional Office State</td>
						</tr>
						<tr class="even">
							<td><input type="checkbox" name="chkBox" id="chkBox"
								value="SALES_POSITION_ID"
								<%if (selList != null) {
				for (int i = 0; i < selList.size(); i++) {
					if ("SALES_POSITION_ID".equals(selList.get(i).toString())) {%>
								checked <%}
				}
			}%>></td>
						...
						<tr class="odd">
							<td><input type="checkbox" name="chkBox" id="chkBox"
								value="SALES_POSITION_TYPE_CD"
								<%if (selList != null) {
				for (int i = 0; i < selList.size(); i++) {
					if ("SALES_POSITION_TYPE_CD".equals(selList.get(i)
							.toString())) {%>
								checked <%}
				}
			}%>></td>
						...
						<tr class="even">
							<td><input type="checkbox" name="chkBox" id="chkBox"
								...
								<%if (selList != null) {
				for (int i = 0; i < selList.size(); i++) {
					if ("HOME_STATE".equals(selList.get(i).toString())) {%>
								checked <%}
				}
			}%>></td>
							<td>Home State</td>
 ...
 %> <input type="hidden" name="newHQSet" value="" />
				<div style="width: 190px; height: 150px;">
					<table class="blue_table" width="100%">
						<tr class="even">
							<td><input type="checkbox" name="chkBoxHQ" id="chkBoxHQ"
								value="DEPTID"
								<%if (selHQList != null) {
					for (int i = 0; i < selHQList.size(); i++) {
						if ("DEPTID".equals(selHQList.get(i).toString())) {%>
								checked <%}
					}
				}%>></td>
							<td>Department ID</td>
						...
						<tr class="odd">
							<td><input type="checkbox" name="chkBoxHQ" id="chkBoxHQ"
								...
								<%if (selHQList != null) {
					for (int i = 0; i < selHQList.size(); i++) {
						if ("EMPLID".equals(selHQList.get(i).toString())) {%>
								checked <%}
					}
				}%>></td>
							<td>Employee ID</td>
						...
						<tr class="even">
							<td><input type="checkbox" name="chkBoxHQ" id="chkBoxHQ"
								...
								<%if (selHQList != null) {
					for (int i = 0; i < selHQList.size(); i++) {
						if ("GUID".equals(selHQList.get(i).toString())) {%>
								checked <%}
					}
				}%>></td>
							<td>GUID</td>
						...
						<tr class="odd">
							<td><input type="checkbox" name="chkBoxHQ" id="chkBoxHQ"
								...
								<%if (selHQList != null) {
					for (int i = 0; i < selHQList.size(); i++) {
						if ("LOCATION".equals(selHQList.get(i).toString())) {%>
								checked <%}
					}
				}%>></td>
					...
					for (int i = 0; i < selHQList.size(); i++) {
						if ("SUPERVISORNAME"
								.equals(selHQList.get(i).toString())) {%>
								checked <%}
					}
				}%>></td>
