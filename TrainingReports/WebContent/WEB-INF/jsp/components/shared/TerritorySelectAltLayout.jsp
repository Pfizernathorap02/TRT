...
<script type="text/javascript" language="JavaScript">	
	function getNextLevel(id,level) {
       ...
       // alert(tmpSel);
        ...
        window.location="/TrainingReports/phase/getNextLevel?<%=AppQueryStrings.FIELD_SALES%>=" +selected_text+"&<%=AppQueryStrings.FIELD_SALESVALUE%>="+selected_value+"&<%=AppQueryStrings.FIELD_SALESORG%>="+selected_sales_text+"&<%=AppQueryStrings.FIELD_SALESLEVEL%>="+level;     
    }
</script>
		
<%-- 	Commented for TRT Phase 2 Enhancement	
<div class="chartscontrol_area">
	  <table class="basic_table" border="2" width="0">
		<form action="<%=wc.getPostUrl()%>" method="post" class="form_basic">
        <%if(tff.getSalesOrgList().size()>2){%>  
        <tr>
                <td nowrap><img src="<%=AppConst.IMAGE_DIR%>/spacer.gif" width="10"></td><td><div class="chartscontrol_areaHdr"><h6>Sales Org:</h6></div></td>                
                <td>
        ...
        <%}else{%>
            ...
            <%}%>
	                ...
	                <select id="<%=wc.getUserFilterForm().FIELD_LEVEL1%>"  name="<%=wc.getUserFilterForm().FIELD_LEVEL1%>" onchange="return getNextLevel(this,2)"> 
                                 ...
                                 <%System.out.println("Geo Filter Form value"+tff.getLevel1());%>
							</select>
						</td>
					</tr>
