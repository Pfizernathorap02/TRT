...
<%            
	ActivitySearchWc wc = (ActivitySearchWc)request.getAttribute(ActivitySearchWc.ATTRIBUTE_NAME);
...
<form class="form_basic" method="post" action="/TrainingReports/adminHome/editNode?type=<%=wc.getType()%>&id=<%=wc.getPhase().getTrackPhaseId()%>">
<table />
<tr>
    <td colspan="2">
        Name
...
%>
            ...
            <form action="/TrainingReports/adminHome/editReport?type=<%=wc.getType()%>&track=<%=wc.getPhase().getTrackId()%>" method="post" onsubmit="this.submit();window.close();" target="main" >
            <table CLASS="blue_table" width="90%">
            <tr >
                <td width="50">
