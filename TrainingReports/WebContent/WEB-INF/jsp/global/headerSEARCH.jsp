...
<%
	HeaderSEARCHWc wc = (HeaderSEARCHWc)request.getAttribute(HeaderSEARCHWc.ATTRIBUTE_NAME);
...
<div id=top_header>
	...
	<div id=header_title><%=(mode1.equalsIgnoreCase("PDF")?"POWERS Driving Force":"Steere Path Forward")%> Search</div>
	
	<UL id=header_top_nav>                                     
        <%--  Infosys code changes starts here
        <LI><a href="<%=AppConst.APP_ROOT%>/reportselect.do">Home</a></LI>
		<LI ><a href="<%=AppConst.APP_ROOT%>/searchPDFHS.do?m0=search&m1=<%=mode1%>&m2=<%=mode2%>">Search</a></LI>                
        --%> 
        <LI><a href="<%=AppConst.APP_ROOT%>/reportselect">Home</a></LI>
		<LI ><a href="<%=AppConst.APP_ROOT%>/searchPDFHS?m0=search&m1=<%=mode1%>&m2=<%=mode2%>">Search</a></LI>                
		...
		<div id=header_bottom_nav>
			<table id=header_nav_table>
				<tr> 
					...
					<% }  else { 
                    if(mode1.equalsIgnoreCase("PDF")&mode2.equalsIgnoreCase("PLC")){%>
                        <td><a href="<%=AppConst.APP_ROOT%>/PWRA/getFilteredChartPLC">1. Product Summary</a>&nbsp;&nbsp;</td>				                                               
                    <%}else if(mode1.equalsIgnoreCase("PDF")&mode2.equalsIgnoreCase("HS")){%>
                        <td><a href="<%=AppConst.APP_ROOT%>/PWRA/getFilteredChart">1. Product Summary</a>&nbsp;&nbsp;</td>				                                               
                    <%}else if(mode1.equalsIgnoreCase("SPF")&mode2.equalsIgnoreCase("PLC")){%>
                        <td><a href="<%=AppConst.APP_ROOT%>/SPF/getFilteredChartPLC">1. Product Summary</a>&nbsp;&nbsp;</td>				                                                
                    <%}                          
                    %>						
					<% } %>
