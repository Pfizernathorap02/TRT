...
public class SceHandler {
	...
	private Sce[] getSalesCallEvaluation( String criteria ) {
		...
		try {
			...
			String sql =	
					" select distinct " + 
						" fe.emplid as emplid, " + 
						//" decode(sce.overall_rating,'DC','" + Sce.STATUS_DC + "','NI','" + Sce.STATUS_NI + "','UN','" + Sce.STATUS_UN + "','" + Sce.STATUS_NOT_COMPLETE + "' ) as rating " + 
                         " (select distinct score_value from sales_call.scoring_system where score_legend = sce.overall_rating) as rating " + //shinoy added on Dec 2nd 2011
					" from  " + 
						" mv_training_required ep, " + 
						" mv_field_employee_RBU fe, " + 
						" (select * from sce_fft where event_id = " + AppConst.EVENTID_FFT + ") sce " + 
					 " where  " + 
						" fe.emplid = ep.emplid " + 
		...
		try {
			...
			String sql =	
					" select distinct " + 
						...
						" (select * from sce_fft where event_id = " + AppConst.EVENTID_FFT + ") sce ," + 
                        //Added by Amit to get Evaluator Name who are not v_new_field_employee but are there in SCE_USER
                        "  sce_users fe_eval1  "+
					 " where  " + 
						" fe.emplid = ep.emplid " + 
						" and ep.emplid = sce.emplid(+) " + 
	...
	/*public SceFull[] getSalesCallEvaluationFull2( String emplid, String productCd, String eventId ) {
		SceFull[] ret = null;
		ResultSet rs = null;
		Statement st = null;
		Connection conn = null;
    ...
    public SceFull[] getSalesCallEvaluationFull2( String emplid, String productCd, String eventId ) {
		...
		try {
			...
			String sql =	
					" select distinct " + 
					 ...
					 " where  " + 
                        ...
                        " and sce.submitted_by_emplid = fe_eval1.emplid(+) "+
						" and fe.emplid = '" + emplid + "' " +
						" and sce.product_cd = '" + productCd + "' order by evaldate desc";
			...
			while (rs.next()) {
				...
				//curr.setRating( getRatingFullForm(conn, rs.getString("rating".toUpperCase()))); /////shinoy added
				curr.setRating( rs.getString("rating".toUpperCase()));
    ...
    private String getRatingFullForm(Connection conn, String ratingShortForm){
		...
		try {
			...
			String sql = " select distinct score_value from sales_call.scoring_system where score_legend = '"+ratingShortForm.trim()+"'";
			//System.out.println("sql:"+sql);
            st = conn.prepareCall(sql); 
            rs = st.executeQuery(); System.out.println("rs:"+rs);
            while (rs.next()) {System.out.println("rs.getString'score_value':"+rs.getString("score_value"));
		...
		try {
			...
			String sql =	
					" select distinct " + 
                        ...
                        " where " +
                        " allSCE.event_id = latestSCE.event_id and " +
    ...
    public int getFeedbackTrackMapping(String trackid)
    {
		...
		try {
			...
			String sql =	
					" select count(*) from feedback_track_mapping" +               
                    " where  " + 
                     "track_id = '" + trackid + "' ";
     ...
     public int getFeedbackActivityMapping(String activityId)
    {
		...
		try {
			...
			String sql = "select  count(*) from p2l_track_phase b, "+
                        ...
                        " FROM mv_usp_activity_hierarchy WHERE activity_pk = '"+activityId+"'" +
    ...
    public static boolean isLMSMapped(String emplid, String productCd, String eventId, String scoreValue ){
		...
		try {
            ...
            String sql = "select distinct template_id from sales_call.template_version where template_version_id IN (select template_version_id from sales_call.sce_fft where emplid = '"+emplid+"' and  product_cd = '"+productCd+"'and event_id = "+eventId+")";
			//System.out.println("sql:"+sql);
            st = conn.prepareCall(sql); 
			...
			while (rs.next()) {System.out.println("rs.getString'template_id':"+rs.getString("template_id"));
				template_id = new Integer(rs.getString("template_id")).intValue();
            }
            System.out.println("template_id:"+template_id);
            sql = "select current_version from sales_call.template where template_id = "+template_id;
            System.out.println("sql2:"+sql);
            st = conn.prepareCall(sql); 
            rs = st.executeQuery();
            //System.out.println("rs2:"+rs);
            while (rs.next()) {
				current_version = new Integer(rs.getString("current_version")).intValue();
            }
            System.out.println("current_version:"+current_version);
            sql = "select template_version_id from sales_call.template_version where version = "+current_version+" and template_id = "+template_id;
            System.out.println("sql3:"+sql);
            st = conn.prepareCall(sql); 
            rs = st.executeQuery();
            System.out.println("rs3:"+rs);
            while (rs.next()) {
				template_version_id = new Integer(rs.getString("template_version_id")).intValue();
            } 
            System.out.println("template_version_id:"+template_version_id);
            sql = "select lms_flag from sales_call.evaluation_form_score where template_version_id = "+template_version_id+" and Score_legend = '"+scoreValue+"'";
    ...
    public static boolean isLMSMapped1( String emplid,String productName,String eventId, String scoreValue){
        System.out.println("isLMSMapped starts....:");
        boolean lmsMapped = false;
        ResultSet rs = null;
		PreparedStatement st = null;
		...
		try {
            ...
            sql="(SELECT max(template_version_id)from sales_call.sce_fft where overall_rating='"+scoreValue+"' and event_id='"+eventId+"' and"+
            " emplid='"+emplid+"' and product_name='"+productName+"')";
            }
            //sql="(SELECT max(template_version_id)from sales_call.template_version tv where template_id=(select distinct(evaluation_template_id)"+
            //"from sales_call.lms_course_mapping where lms_course_name='"+productName+"')and tv.Publish_flag='Y')";
            ...
            int current_version = -1;
             while (rs.next()) {
				template_version_id = new Integer(rs.getString("max(template_version_id)")).intValue();
            }
			//while (rs.next()) {System.out.println("rs.getString'template_id':"+rs.getString("template_id"));
			//	template_id = new Integer(rs.getString("template_id")).intValue();
            //}
    ...
    public static String getScoreLegend(String scoreValue){
		...
		try {
			...
			Infosys code changes ends here
			 */ 
            String sql = "select distinct score_legend from  sales_call.scoring_system where score_value = trim('"+scoreValue+"')";
			//System.out.println("sql:"+sql);
            st = conn.prepareCall(sql); 
            //System.out.println("st:"+st);
			rs = st.executeQuery(); //System.out.println("rs:"+rs);
            while (rs.next()) {System.out.println("rs.getString'template_id':"+rs.getString("score_legend"));
     ...
     public static String getNtId(String role,String emplId, String email){
		...
		try {
            ...
            {
                sql = "select distinct nt_id from mv_field_employee_rbu where emplid = '"+emplId+"'";
            }
            else if(role != null && role.equals("user"))
            {
                //sql = "select distinct nt_id from user_access where emplid = '"+emplId+"'";
                sql = "select distinct nt_id from user_access where  emplid = '"+emplId+"' OR LOWER(email) = '"+email+"'";
                //select * from user_access where  emplid = '' OR LOWER(email) = 'vivian.armstrong@pfizer.com';
            }
            //System.out.println("sql:"+sql);
            st = conn.prepareCall(sql); 
            //System.out.println("st:"+st);
    ...
    public static boolean isCourseMapped(String productName){
		...
		try {
            ...
            sql = "select lms_course_name from sales_call.lms_course_mapping where lms_course_name='"+productName+"'";
            st = conn.prepareCall(sql); 
            //System.out.println("st:"+st);
			rs = st.executeQuery(); //System.out.println("rs:"+rs);
       ...
       return false;
		...
		try {
			 ...
			 */	
            sql = "select lms_course_name from sales_call.lms_course_mapping where lms_course_name='"+productName+"'"
            +" union select product_name from sales_call.sce_fft where product_name='"+productName+"' and event_id not in (5,6,7)";
            st = conn.prepareCall(sql); 
            //System.out.println("st:"+st);
			rs = st.executeQuery(); //System.out.println("rs:"+rs);
