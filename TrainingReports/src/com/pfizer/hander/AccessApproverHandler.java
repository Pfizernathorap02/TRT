...
public class AccessApproverHandler 
{
	...
	public void updateApprovers(AccessApproversMembers updatedMember)
	{
	      ...
	      {
	         tx = session.beginTransaction();
	         Query hql = session.createQuery("update AccessApproversMembers set emailId=? where approverType=?");
	         hql.setParameter(0,updatedMember.getEmailId());
	         hql.setParameter(1,updatedMember.getApproverType());
	         int count = hql.executeUpdate();
	         System.out.println(count+" approver updated");
	         tx.commit();
	      }
	      catch (HibernateException e) 
	      {
	...
	public String[] getAccessApproversEmails()
	{
	      ...
	      {
	         tx = session.beginTransaction();
	         List<AccessApproversMembers> approvers = session.createQuery("FROM AccessApproversMembers").list(); 
	         tx.commit();
	         
	         for (AccessApproversMembers members : approvers) 
	         {
	        	 if(addresses == null || addresses.length()==0 ||addresses.equalsIgnoreCase(""))
