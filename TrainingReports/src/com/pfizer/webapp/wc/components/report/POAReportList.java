...
public class POAReportList extends WebComponent 
{ 
  ...
  public static final int LAYOUT_EXCEL	= 2;
    ...
    
    {
        ...
        return AppConst.JSP_LOC + "/components/report/POAReportList.jsp";
    }
    ...
    POAReportList(UserFilter filter,Employee[] emplBean,User user){
     ...
     POAReportList(UserFilter filter,Employee[] emplBean,User user,boolean excel){
         System.out.println("EmplBean length here is "+emplBean.length);
         qStrings = filter.getQuseryStrings();
         this.user=user;
        setEmployeeBean(emplBean);
